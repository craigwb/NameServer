			...
			if (ID > 0) { // not bootstrap
				...
				while (!quit) {
					...
					if (userIn.equalsIgnoreCase("enter") && !entered) {
						...
						System.out.println("Succesfull Enter");
						System.out.println("Keys stored: " + in.readLine());
						System.out.println("ID of Predecor: " + in.readLine());
						System.out.println("ID of Successor: " + in.readLine());
						System.out.println("The bootstrap server was contacted and so were servers: " + in.readLine());
			...
			try {
				...
				while (!quit) {
					try {
						...
						out = new PrintWriter(servers[succ].getOutputStream(), true); // cleans up succesors keys
						out.println("delete");
						out.println(ID);
					} catch (Exception e) {
					}
				...
				try {
					...
					if (in.equals("exit")) {
						...
						if (succ == 1024) {
							...
							BufferedReader succ_in = new BufferedReader(
									new InputStreamReader(servers[succ].getInputStream()));
			...
			try {
				...
				while (!quit) {
					...
					if (s.equals("append")) {
						...
						System.out.println(
								"Succesor: " + succ_in.readLine() + " Range handed over: " + succ_in.readLine());
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
